...
class BeakerMachine(object):
    ...
    def __init__(self, url, username, password, fqdn, web_service, ca_cert, custom_loan_comment,
        ...
        if not custom_loan_comment:
            self.custom_loan_comment = DEFAULT_LOAN_COMMENT
        else:
            self.custom_loan_comment = custom_loan_comment
    ...
    def _loan_system(self):
        ...
        resp = self.session.post(url, headers=headers,
                                 data=json.dumps({'comment': self.custom_loan_comment}))
