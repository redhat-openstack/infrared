...
# Creates amphora image for the same distribution type as the current
# operating system.
AMP_DISTRO=
...
then
    ...
    distro=`awk -e '{print $1;}' /etc/redhat-release`
    case "$distro" in
        CentOS)
            AMP_DISTRO="centos"
            ;;
        Red)
            AMP_DISTRO="rhel"
            echo "Selecting RHEL - did you configure the environment to"
            echo "select the proper base image and Portal/Satellite"
            echo "registration?"
            ;;
        *)
            echo "Unsupported distribution"
...
sudo yum install -y openstack-octavia-diskimage-create

octavia-diskimage-create.sh -i $AMP_DISTRO -s 3

if [ ! -e amphora-x64-haproxy.qcow2 ];
then
    echo "Unable to find expected Amphora image! Did image creation fail?"
    exit 1
fi

glance image-create --name octavia-amphora --disk-format qcow2 \
    --container-format bare --tags amphora --file amphora-x64-haproxy.qcow2
...
# WARNING! For testing a development ONLY! Do not use certificates
# generated by this script in production!
mkdir private newcerts
touch index.txt
echo 01 > serial
# You may need to modify this line if your distribution puts the
# system openssl.cnf in a different directory.
cp /etc/pki/tls/openssl.cnf .
sed -i -e "s/^dir\\s.*$/dir = ./" -e "s/cacert.pem/ca_01.pem/" openssl.cnf
...
openssl req -x509 -passin pass:foobar -new -nodes -key private/cakey.pem \
        ...
        -out ca_01.pem
openssl req  -newkey rsa:2048 -nodes -config ./openssl.cnf -keyout client.key \
       -subj "/C=US/ST=Denial/L=Springfield/O=Dis/CN=www.example.com" \
       -out client.csr

openssl ca -config ./openssl.cnf -passin pass:foobar -in client.csr \
           -days 365 -out client-.pem -batch
...
EOF_BR_INT
    ...
    cat << EOF_PORT_CFG | \
        ssh $SSH_ARGS heat-admin@$node "sudo bash -c \"cat > /etc/sysconfig/network-scripts/ifcfg-$MGMT_PORT_DEV\""
TYPE=OVSIntPort
OVS_BRIDGE=br-int
DEVICETYPE=ovs
BOOTPROTO=static
...
ONBOOT=yes
EOF_PORT_CFG
   ssh $SSH_ARGS heat-admin@$node sudo ifup $MGMT_PORT_DEV

   # Bring the interface up.
   ssh $SSH_ARGS heat-admin@$node sudo iptables -I INPUT -i$MGMT_PORT_DEV -p udp --dport 5555 -j ACCEPT
