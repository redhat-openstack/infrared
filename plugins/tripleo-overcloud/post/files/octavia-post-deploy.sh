...
do
    ...
    ssh -q heat-admin@$node 'sudo mkdir -p /etc/octavia/.ssh'
    ssh heat-admin@$node 'sudo mkdir -p /etc/octavia/certs/private'

    cat octavia_ssh_key | ssh heat-admin@$node 'sudo bash -c "cat > /etc/octavia/.ssh/octavia_ssh_key"'
    cat octavia_ssh_key.pub | ssh heat-admin@$node 'sudo bash -c "cat > /etc/octavia/.ssh/octavia_ssh_key.pub"'

    cat client.pem | ssh heat-admin@$node 'sudo bash -c "cat > /etc/octavia/certs/client.pem"'
    cat ca_01.pem | ssh heat-admin@$node 'sudo bash -c "cat > /etc/octavia/certs/ca_01.pem"'
    cat private/cakey.pem | ssh heat-admin@$node 'sudo bash -c "cat > /etc/octavia/certs/private/cakey.pem"'

    # Make sure permissions are correct!
    ssh heat-admin@$node 'sudo chown -R octavia:octavia /etc/octavia/.ssh/*'
    ssh heat-admin@$node 'sudo chown -R octavia:octavia /etc/octavia/certs/*'
    ...
    cat << EOF_BR_INT | \
        ssh heat-admin@$node "sudo bash -c \"cat > /etc/sysconfig/network-scripts/ifcfg-br-int\""
DEVICETYPE=ovs
TYPE=OVSBridge
BOOTPROTO=none
DEVICE=br-int
...
EOF_BR_INT
    ...
    cat << EOF_PORT_CFG | \
        ssh heat-admin@$node "sudo bash -c \"cat > /etc/sysconfig/network-scripts/ifcfg-$MGMT_PORT_DEV\""
TYPE=OVSIntPort
OVS_BRIDGE=br-int
DEVICETYPE=ovs
BOOTPROTO=static
...
ONBOOT=yes
EOF_PORT_CFG
   ssh heat-admin@$node sudo ifup $MGMT_PORT_DEV

   # Bring the interface up.
   ssh heat-admin@$node sudo iptables -I INPUT -i$MGMT_PORT_DEV -p udp --dport 5555 -j ACCEPT
   ...
   cat << ENDOFWORKER | \
       ssh heat-admin@$node "sudo bash -c \"cat > /etc/octavia/conf.d/octavia-worker/worker-post-deploy.conf\""
[controller_worker]
...
ENDOFWORKER
   cat << HLTHMGR | \
       ssh heat-admin@$node "sudo bash -c \"cat > /etc/octavia/conf.d/octavia-health-manager/manager-post-deploy.conf\""
[health_manager]
bind_ip = $MGMT_PORT_IP
HLTHMGR
   cat << NEUTRON_XTRA | \
       ssh heat-admin@$node "sudo bash -c \"cat > /etc/neutron/conf.d/neutron-server/octavia-post-deploy.conf\""
[service_auth]
...
NEUTRON_XTRA
   cat << OCTAVIA_XTRA | \
       ssh heat-admin@$node "sudo bash -c \"cat > /etc/octavia/conf.d/common/octavia-post-deploy.conf\""
...
do
    ...
    echo "controller_ip_port_list = $CONTROLLER_IP_LIST" | \
       ssh heat-admin@$node "sudo bash -c \"cat >> /etc/octavia/conf.d/octavia-health-manager/manager-post-deploy.conf\""
    ssh heat-admin@$node "sudo systemctl restart octavia-*"
    ssh heat-admin@$node "sudo systemctl restart neutron-*"
done
