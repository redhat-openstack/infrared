...
function help() {
    echo "$(basename "$0") [ -b ] [ -t ] [ -f ] -- script to generate release nodes between two git branches or commits"
    echo "where:"
    ...
    echo "    -f/--format       providing option to specify output format, text or markdown"
    echo ""
    echo "Example: ./$(basename "$0") -b stable master -t FIX -f markdown"
...
for tag in `echo ${TAGS} | tr "," "\n"`; do
    ...
    for commit in `git log ${FROM}...${TO} --pretty='%h %s' | awk -v tag="[$tag]" '$2 == tag { print $1 }'`; do
        ...
        commit_body=$(git show -s --format='%b' $commit | grep -vE 'Change-Id|cherry picked from commit' | sed '${/^[[:space:]]*$/d;}')
