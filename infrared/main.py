    ...
    if 'VIRTUAL_ENV' in os.environ and \
        ...
        if not config.has_option('ssh_connection', 'control_path_dir') \
           and not config.has_option('ssh_connection', 'control_path'):
            ...
            cp = os.path.join(os.environ['VIRTUAL_ENV'], '%%C')
            if len(os.environ['VIRTUAL_ENV']) > 60:
                # better not to anything if path is too long, but inform user
                LOG.warn("Avoided setting ANSIBLE_SSH_CONTROL_PATH to "
                         "%s because path lengh > 60 chars "
                         "which may break linux socket limit of ~100 "
                         "chars." % cp)
            else:
                LOG.warn("Set ANSIBLE_SSH_CONTROL_PATH=%s to avoid "
                         "eventual ssh session errors or hickjaking." % cp)
                os.environ['ANSIBLE_SSH_CONTROL_PATH'] = cp
