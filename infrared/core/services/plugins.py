...
class InfraredPluginManager(object):
    ...
    def add_plugin(self, plugin_source, dest=None):
        ...
        :param plugin_source: Plugin source.
          Can be:
            1. Plugin name to install from the registery (project's plugins)
            2. Path to a local directory
            3. Git URL
        :param dest: destination where to clone a plugin into (
        """
        ...
        if os.path.exists(project_plugin_dir) and \
            ...
            if not os.path.exists(plugin_source):
                ...
                os.chdir(tmpdir)
                try:
                    subprocess.check_output(["git", "clone", plugin_source])
                except subprocess.CalledProcessError:
                    shutil.rmtree(tmpdir)
                    raise IRFailedToAddPlugin(
                        "Cloning git repo {} is failed".format(plugin_source))
                cloned = os.listdir(tmpdir)
                plugin_dir_name = cloned[0]

                plugin_source = os.path.join(destination, plugin_dir_name)
