...
class CoreServices(object):
    ...
    def setup(cls, file_path='infrared.cfg', section='core'):
        ...
        ws_base_folder = os.path.expanduser(
            config.get(section, 'workspaces_base_folder'))
        if not os.path.isabs(ws_base_folder):
            ws_base_folder = os.path.abspath(
                os.path.join(ir_dir, ws_base_folder))

        plugins_conf_file = os.path.expanduser(
            config.get(section, 'plugins_conf_file'))
        if not os.path.isabs(plugins_conf_file):
            plugins_conf_file = os.path.abspath(
                os.path.join(ir_dir, plugins_conf_file))
