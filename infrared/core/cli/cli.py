...
class CliParser(object):
    ...
    def parse_cli_input(cls, arg_parser, args=None):
        ...
        # custom ansible arg structure: ansible<arg_name>
        custom_ans_expr = '^(?P<subcmd_name>ansible)+(?P<arg_name>.*)$$'
        for arg, value in parse_args.items():
            ...
            if match and match.groupdict().get('arg_name', None):
                ...
                # we have ansible. popule a dict
                arg_name = custom_ans_match.group('arg_name')
                custom_result[arg_name] = value
    ...
    def _add_argument(cls, spec, subparser, group, option_name, option_data,
        ...
        if 'ansible_variable' in option_data:
            opt_kwargs['dest'] = option_data.get(
                'dest', 'ansible' + option_data['ansible_variable'])
