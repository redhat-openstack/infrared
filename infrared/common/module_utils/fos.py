...
class ClientContext(object):
    ...
    # simple os request: json in, json out
    ...
    def compute_raw_server_search(self, extra_params=None, all_projects=False):
        url = '/servers/detail'
    ...
    def compute_server_search(self, server=None, filters=None, all_projects=False):
        # TODO(afazekas): detaild
        # TODO(afazekas): advanced filtering
        response = self.compute_raw_server_search(filters, all_projects)
        openstack_servers = response['servers']
        if server:
            openstack_servers = [srv for srv in openstack_servers
                                 if fnmatch.fnmatch(srv['name'], server) or fnmatch.fnmatch(srv['id'], server)]
        # old client copied <group>:<arg> to <arg>
        for srv in openstack_servers:
            for k in srv.keys():
                if ':' in k:
                    nk = ''.join(k.split(':')[1:])
                    if nk not in srv:
