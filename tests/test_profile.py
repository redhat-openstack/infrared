...
def test_profile_export(profile_manager_fixture, test_profile, mocker):
    mock_tarfile = mocker.patch.object(profiles, "tarfile")
    mock_tarfile.open.return_value = mocker.MagicMock()
    mock_os = mocker.spy(profiles, "os")

    back_get = profile_manager_fixture.get
    profile_manager_fixture.get = lambda x: test_profile
    profile_manager_fixture.export_profile(test_profile.name, "some_file")
    profile_manager_fixture.get = back_get
    mock_tarfile.open.assert_called_with("some_file.tgz", "w:gz")
    mock_os.path.join.assert_called_with(test_profile.path, "")

    back_get = profile_manager_fixture.get
    profile_manager_fixture.get = lambda x: test_profile
    profile_manager_fixture.export_profile(test_profile.name, None)
    profile_manager_fixture.get = back_get
    mock_tarfile.open.assert_called_with("{}.tgz".format(test_profile.name),
                                         "w:gz")
    mock_os.path.join.assert_called_with(test_profile.path, "")
    ...
    profile_manager_fixture.activate(test_profile.name)
    profile_manager_fixture.get = lambda x: test_profile
    profile_manager_fixture.export_profile(None, None)
    profile_manager_fixture.get = back_get
    mock_tarfile.open.assert_called_with("{}.tgz".format(test_profile.name),
                                         "w:gz")
    mock_os.path.join.assert_called_with(test_profile.path, "")
...
def test_profile_export_not_excist(profile_manager_fixture):
    with pytest.raises(exceptions.IRProfileMissing):
        profile_manager_fixture.export_profile("some_profile", None)


def test_profile_export_no_active(profile_manager_fixture):
