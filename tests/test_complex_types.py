...
def test_file_type_resolve(file_root_dir, file_type, monkeypatch):
    ...
    assert file_type.resolve('file1') == file_root_dir.join('file1.yml').strpath
    assert file_type.resolve(os.path.abspath('file1')) == file_root_dir.join(
        'file1.yml').strpath
...
def test_list_of_files(file_root_dir, list_file_type, monkeypatch):
    """Verifies the list of files"""
    monkeypatch.setattr("os.getcwd", lambda: file_root_dir.strpath)

    assert list_file_type.resolve('file1') == [
        file_root_dir.join('file1.yml').strpath]
    assert list_file_type.resolve('file1,file2') == [
        file_root_dir.join('file1.yml').strpath,
