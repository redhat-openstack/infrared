...
# def workspacedir(tmpdir_factory, request):
#     mytemp = tmpdir_factory.mktemp("workspaces")
#     request.addfinalizer(lambda: mytemp.remove())
#     workspace.WorkspaceManager.workspacedir = mytemp.strpath
...
@pytest.fixture(scope="function")
def workspacedir(mock_spec):
    workspace.WorkspaceManager._workspacedir = None
    workspace.WorkspaceManager._config = mock_spec.config


@pytest.mark.parametrize('inventory_content', ["fake content", ""])
def test_workspace_inventory(tmpdir, inventory_content, workspacedir):
...
#
# def test_execute(tmpdir, workspacedir):
...
def test_create_list_use_workspace(workspacedir):
    wkpsc_name = "workspace_test"
...
def test_active_workspace(workspacedir):
