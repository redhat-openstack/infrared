    ...
    poodle_type:
        description:
            - specifies the poodle to use. Should be used with
             (C(enable_poodle_repos)) set to true. Implies (C(puddle)) value.
...
def _parse_output(module, stdout):
    ...
    if len(installed_releases) > 2 or (
                    len(installed_releases) == 2 and set(
                r["channel"] for r in installed_releases) != {"product",
                                                              "core"}):
        module.fail_json(
            msg="Can't handle more than 2 channels. 1 core, 1 ospd. Found %s" % installed_releases)
...
def _run_command(module, cmd):
    ...
    module.exit_json(changed=True, **wrap_results(summary, cmd, rc, out, err))
...
def main():
    ...
    module = AnsibleModule(
        argument_spec=dict(
            state=dict(default="install", choices=['install', 'uninstall', 'update']),
            release=dict(),
            puddle=dict(),
            pin_puddle=dict(default=False),
            enable_poodle_repos=dict(default=False),
            poodle_type=dict(choices=POODLE_TYPES.keys()),
            target_directory=dict(default='', required=False),
            distro_version=dict(),
            override_hostname=dict(),
    ...
    override_hostname = module.params['override_hostname']
    ...
    override_hostname = ['-H', override_hostname] if override_hostname else []
    ...
    if state == 'uninstall':
        ...
        cmd.extend(override_hostname)
        cmd.extend(one_shot_mode)
        cmd.extend(repo_args)

    _run_command(module, cmd)
