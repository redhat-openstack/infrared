...
commitMsgBody_excludeLines="Change-Id|Related|Related-Bug|Related-Change|Related-To|Resolves|Side-change|Signed-off-by|Solves|Tested-by|CIX|JIRA"
commitMsgBody="$(git log --no-merges --pretty="%b" -1)"
if ! grep -Ev "^\s*$|^(${commitMsgBody_excludeLines}):.*$" <<<"${commitMsgBody}"; then
    (set +x;
