...
* FileValue
    The absolute or relative path to a file. Infrared validates whether file exists and
    passes absolute path forward to vars_dict.

* VarFile
    Same as the ``FileValue`` type but additionally Infrared will the check the following locations for a file:
        - ``<plugin_root>/argument/name/option_value``
        - ``<plugin_root>/defaults/argument/name/option_value``
        - ``<plugin_root>/var/argument/name/option_value``

    In that example above the CLI option name is ``--argument-name``.
    The ``VarFile`` suites very well to describe an option which points to the file with variables.

    For example, user can describe network topologies parameters in separate files.
    In that case, all these files can be put to the ``<plugin_root>/defaults/network`` folder,
    ...
    That variable can later be used in Ansible playbooks to load the appropriate network parameters.
