...
There is an option to override the default Open vSwtich shipped with the Overcloud image by supplying a Fast DataPath(FDP) YUM repo file and a version string to be used::

  infrared tripleo-undercloud --images-task rpm --images-update yes --fdp-ovs-rpm-string '2.11' --fdp-repo-file-url http://path.to.fdp.repo/fast-datapath-staging.repo
