...
There is an option to override the default Open vSwtich shipped with the Overcloud image by supplying a FDP YUM repo file and a version string to be used::

  infrared tripleo-undercloud -o undercloud-images.yml --images-task rpm --images-update yes --build passed_phase2 --mirror tlv --fdp-ovs-rpm-string '2.11' --fdp-repo-file-url http://path.to.fdp.repo/fast-datapath-staging.repo

.. note:: both 'fdp-ovs-rpm-string' and 'fdp-repo-file-url' arguments are required in order to install a custom OVS release
