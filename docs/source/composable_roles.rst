...
To deploy overcloud with the composable roles the additional tempalates should be provided:
  ...
  - template with the infomration about roles count, flavors and other defaults::
...
InfraRed allows to simplify the porcess of templates generation and auto-populates the roles according to the deployed topology.


Defining topology and roles
===========================

In order to deploy custom roles, the InfarRed should know what nodes should be used for what roles.

There is a 2-step procedure to achive that.

**Step #1** Setup availables nodes and store them in the Infared inventory. Those nodes can be configured by the ``prvision`` pluing such as `virsh <virsh.html>`_::
...
In that example, to build the composable roles templates, InfraRed will look into the ``<plugin_dir>/vars/role/networker`` folder
for the files that corresponds to all the node names defined in the ``invnetory->overcloud_nodes`` group.
...
InfraRed will try to use a ``default`` roles from the ``<plugin_dir>/vars/role/default`` folder.

For the topology described above with the networker custom role the follwing role files can be defined:
  - <plugin_dir>/vars/role/**networker**/controller.yml - holds controller roles without neutron services
  - <plugin_dir>/vars/role/**networker**/netwroker.yml - holds the networker role description with the neutron services
  - <plugin_dir>/vars/role/**default**/compute.yml - a default compute role description
  - <plugin_dir>/vars/role/**default**/swift.yml - a default swift role description

To deploy non-supported roles, a new folder should be created in the ``<plugin_dir>/vars/role/``.
Any roles files that differ (e.g. service list) from the defaults should be put there. That folder is then can be referenced with the ``--role-files=<foldername>`` argument.

Role Description
=================

All the custom and defaults role descriptions are stored in the ``<plugin_dir>/vars/role`` folder.
Every role file holds the follwing infromation:
  - ``name`` - name of the role
  - ``resource_registry`` - all the resources required for a role.
  - ``flavor`` - the flavor to use for a role
  - ``host_name_format`` - the resuting host name format for the role node
  - ``services`` - the list of services the role holds

Below is an example of the controler default role::
...
The name of the role files should correspond to the node invntory name without prefix and index.
