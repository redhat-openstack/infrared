...
Sample plugin specification file can like like::
...
Options::
    ...
    * ``include_groups``: list what standard flags should be included to the plugin CLI.
        ...
        get ``--extra-vars``, ``--output`` and ``--drr-run``. See `plugins include groups`_ for more information.
    * ``groups``: the list of options groups
        Groups several logically connected options.
    * ``options``: the list of option in a group.
