...
2. Install: InfraRed will notice that no UC is provided and will build one from a snapshot of an installed UC from latest available build. InfraRed will recognize that there is no undercloud node in the Ansible's inventory file and will run in 'quickstart' mode automatically. (Ansible inventory file contains a list of available hosts and some useful information on them, this file created automatically during the provisioning process)
