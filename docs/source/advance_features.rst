...
Hide Ansible's STDOUT
---------------------
When ``IR_ANSIBLE_NO_STDOUT`` is set, output that comes from Ansible's stdout won't be printed (this doesn't include output from stderr. Please see ``IR_ANSIBLE_STDERR_TO_STDOUT`` below)

Redirect Ansible's STDERR to STDOUT
-----------------------------------
The ``IR_ANSIBLE_STDERR_TO_STDOUT`` environment variable instructs Infrared to redirect Ansible's stderr to stdout.
