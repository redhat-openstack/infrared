...
def print_out(results, verbose):
    if verbose:
        for result in results:
            print(red('FAIL: %s, %s' % (result['dict'])))


def run_tests(plugins, verbose=False):
    any_failure = False
    try:
        if verbose:
        ...
        print(red('ERROR: seems %s is not valid yaml file!') % plugins)
        any_failure = True
    return any_failure
...
if __name__ == '__main__':
    ...
    opts = ('-v')
    verbose = ('-v' in plugins)

    sys.exit(int(run_tests(plugins, verbose)))
