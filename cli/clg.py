...
class SpecDictHelper(object):
    ...
    def __init__(self, spec_dict):
        ...
        for subparser_name, subparser_dict in parser_dict.get(
                'subparsers', {}).items():
            self.include_groups(subparser_dict)
    ...
    def iterate_commands(self):
        ...
        """
        yield self.spec_dict, True
        for subparser_name, subparser_dict in self.spec_dict.get('subparsers',
                                                                 {}).items():
            yield dict(name=subparser_name, **subparser_dict), False

    def get_all_options_spec(self, parser_dict):
        """
        Gets alls the options specification as the list of dicts
        """
        result = []
    ...
    def get_command_options_spec(self, command_name):
        ...
        if command_name == 'main':
            options = self.get_all_options_spec(self.spec_dict)
        else:
            for name, subparser in self.spec_dict.get(
                    'subparsers', {}).items():
