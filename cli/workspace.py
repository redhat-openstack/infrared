...
class WorkspaceManager(object):
    _default_workspacedir = os.path.join(os.path.expanduser("~"), ".infrared",
                                "workspaces")
    _workspacedir = None
    _config = None

    @classmethod
    def _get_workspacedir(cls):
        if cls._workspacedir:
            return cls._workspacedir
        # todo(yfried): improve conf handling
        cls._config = cls._config or conf.ConfigWrapper.load_config_file()
        cls._workspacedir = cls._config.get_workspaces_dir() or \
                            cls._default_workspacedir
    ...
    def create(cls, name="", *args, **kwargs):
        ...
        path = os.path.join(cls._get_workspacedir(), name)
        if os.path.exists(path):
    ...
    def get_active(cls):
        ...
        """
        try:
            with open(os.path.join(cls._get_workspacedir(), ".active")) as wkspc_file:
                name = wkspc_file.read().strip()
                return cls.get(name)
        except exceptions.IRWorkspaceMissing:
            os.remove(os.path.join(cls._get_workspacedir(), ".active"))
        except IOError as e:
            if e.errno != 2:
                raise
    ...
    def _set_active(cls, name):
        with open(os.path.join(cls._get_workspacedir(), ".active"), 'w') as wkspc_file:
