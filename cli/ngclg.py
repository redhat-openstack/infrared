...
class Spec(object):
    ...
    def parse_args(self, allowed_unrecognized_args=None):
        """
        ...
        :param allowed_unrecognized_args: the list args which are
                not specified in spec but are allowed to use.
        """

        spec_defaults = self.get_spec_defaults()
        env_defaults = self.get_env_defaults()
        cli_args, unknown_args = CliParser.parse_args(
            self, allowed_unrecognized_args)
        ...
        unknown_args2 = self.validate_arg_sources(
            cli_args,
            env_defaults,
            file_args,
            spec_defaults)

        # take unknown args as a flat dict.
        unknown_args2 = {nested_key: nested_value
                         for key in cli_args.keys() if key in unknown_args2
                         for nested_key, nested_value
                         in unknown_args2[key].items()}
        utils.dict_merge(unknown_args2, unknown_args)

        if unknown_args2:
            unknown_options = [arg_name for arg_name in unknown_args2
                               if arg_name not in allowed_unrecognized_args]
            if not unknown_options:
                LOG.warning("The following args will be "
                            "passed to Ansible: {}".format(unknown_args2))
            else:
                raise exceptions.IRUnrecognizedOptionsException(
                    unknown_options)
    ...
    def validate_arg_sources(self, cli_args, env_defaults, file_args,
        ...
        for command, command_dict in cli_args.items():
            ...
            unreq_args[command] = self.validate_unrecognized_arguments(
                command, file_dict)
...
class CliParser(object):
    ...
    @classmethod
    def parse_args(cls, spec, additional_args=None):
        parser_dict = spec.spec_helper.spec_dict['command']

        add_arg_note = ''
        if additional_args:
            add_arg_note = "\n\nNote: The following Ansible options " \
                           "can be used along with spec " \
                           "options: \n{}".format(
                            map(lambda x: "--" + x, additional_args))
    ...
    def _transform_unknown_args(uargs):
        ...
        for arg_index, arg in enumerate(uargs):
            ...
            if arg and '=' in arg:
                ...
                skip_next = False
            elif arg and arg[0] in prefix_chars:
                name = arg
